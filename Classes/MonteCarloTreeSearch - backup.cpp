...
/*
	...
	expand possible moves. -> only select based on player's previous behaviour (roll out policy)
...
/*
	Roll out policy = selecting node (candidate) based on game play and user behaviour
*/

MCNode MonteCarloTreeSearch::rollout_policy() 
{
	MCNode selected;
	return selected;
}
...
/*Selection use tree policy to construct path from root to most PROMISING LEAF NODE
	LEAF NODE -> node which has unexplored child node(s)
	SNOWCAP -> explored part of the game tree as opposed to the vast unexplored bottom part
*/
...
/*
	Expansion = randomly pick an unexplored node of a leaf node
...
/*
	Roll out one or multiple simulation with reward accumulated for each simulation.
	Roll-out policy is normally simply or even pure random such that it is fast to execute. 
	For example, a win might induce reward of +1, a draw for 0, and a loss for -1.
*/
...
/*
	Backpropagation / backup = Use to accumulate rewards of simulations to back up 
	and update the value of nodes in the snowcap.
*/
...
SMove MonteCarloTreeSearch::search(MCNode * node, Board board, vector<char> computerRack, vector<vector<int> > leftTiles, SMove latestMove)
{
	...
	leftTiles = basicTiles.updateUnseenLetter(26, leftTiles);
	//node->leftTiles = leftTiles;
//	node->rack = computerRack;
	//node->state = board;
	...
	while (iter < 10 && (iter == 0 || iter < (&tree.getRoot())->child.size()) && now < end)
	{
		...
		//promisingNode->rack = computerRack;
		//promisingNode->leftTiles = leftTiles;
		//2. expansion
