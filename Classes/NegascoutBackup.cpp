...
class Negascout
{
	...
	nodetree* addChild(int eval, nodetree *S,string word,int row,int col,int direction , int score,int leftTiles,int rowAnchor,int colAnchor,float heuristic)
	{
		if(S!=NULL)
		{
			nodetree * baru = new nodetree();
			...
			baru->depth = baru->parent->depth + 1;
			return baru;
		}
		else
		{
	...
	int evaluate(nodetree *position,int depth)
	{
		...
		{	
			...
			if(now->parent == T.root)
			{
				...
				if(now->endEval == -999 || now->endEval < player) 
				{
					now->endEval = player;
					endEval = player;
				}
		...
		for(i = 0 ; i < T.root->totalChild; i++)
		{
			...
			if(now->eval == eval)
			{
				if(now->score >= max)
				{
					if(now->score == max)
					{
						if(now->player >= maxEval)
						{
							if(now->player == maxEval)
							{
								if((now->word).length() > maxLength)
								{
									ketemu = 1;
								}
							}
	...
	void printChild(nodetree* parent)
	{
		...
		{
			std::cout<<"eval "<<now->eval<<" word "<<now->word<<" skor "<<now->score<<" depth "<<now->depth<<" end "<<now->endEval<<" child "<<now->totalChild<<"\n";
			ofile<<"eval "<<now->eval<<" word "<<now->word<<" skor "<<now->score<<" depth "<<now->depth<<" end "<<now->endEval<<" child "<<now->totalChild<<"\n";
			now = now->sibling;
		}
		ofile<<"\n\n\n";
		ofile.close();
